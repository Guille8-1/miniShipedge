...
export default function ProjectsActionsPage() {
  ...
  const hancdleClick = (event: any) => {
    ...
    useEffect(() => {
        if(open){
            window.addEventListener("click", hancdleClick);
        }
        return () => {
            window.removeEventListener("click", hancdleClick);
        }
    }, [open]);
  ...
  return (
    <>
        <section className="h-auto w-full">
          <button onClick={isOpen} className="flex flex-row align-items-center justify-items-center gap-2 bg-sky-800 p-3 text-gray-200 rounded">
